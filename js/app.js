...
async function loadConfig() {
  try {
    ...
    console.warn('[app] No se pudo cargar config.json. Usando valores de respaldo.');
    return { links: FALLBACK_LINKS, server: { status: 'online' } };
  }
}
...
function applyServerStatus(status) {
  ...
  if (!onlinePill && !maintenancePill) return;
...
/* ════ WINDOW EVENTS ════ */
window.addEventListener('resize', () => { resizeStarCanvas(); initStars(); resizeDust(); });
window.addEventListener('scroll', () => { scrollY_ = window.scrollY; }, { passive: true });

/* ════ BOOT ════ */
document.addEventListener('DOMContentLoaded', async () => {
  const config = await loadConfig();
  applyLinks(config.links || FALLBACK_LINKS);
